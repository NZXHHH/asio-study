...
CSession::CSession(boost::asio::io_context& ios, CServer* server) :_soc(ios), _server(server),_strand(ios.get_executor()) {
...
void CSession::Start()
{
	...
	boost::asio::async_read(_soc,boost::asio::buffer(_head_node->_data, HEAD_TOTAL_LEN),
	boost::asio::bind_executor(_strand,bind(&CSession::handle_read, this, placeholders::_1, placeholders::_2, shared_from_this())));
...
void CSession::handle_write(boost::system::error_code error, std::size_t, shared_ptr<CSession> session)
{
	if (!error)
	{
		...
		if (!que.empty())
		{
			...
			boost::asio::bind_executor(_strand,bind(&CSession::handle_write, this, placeholders::_1, placeholders::_2, shared_from_this()
				)));
		}
	}
	else
...
void CSession::handle_read(boost::system::error_code error, std::size_t tranfered, shared_ptr<CSession> session)
{
	if (!error)
	{
		...
		boost::asio::async_read(_soc, boost::asio::buffer(_recv_msg_node->_data, body_len),
		
		boost::asio::bind_executor(_strand,bind(&CSession::HandleReadMsg, this, placeholders::_1, placeholders::_2, shared_from_this())));
